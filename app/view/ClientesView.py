...
class Cliente_View(QWidget, Ui_ControlCliente):
    ...
    def __init__(self, parent=None):
        ...
        self.TablaClientes.cellClicked.connect(
        ...
        self.InputNombre.returnPressed.connect(
            self.editar_cliente
        )

        self.InputApellido.returnPressed.connect(
            self.editar_cliente
        )

        self.InputTelefono.returnPressed.connect(
            self.editar_cliente
        )

        self.InputDireccion.returnPressed.connect(
            self.editar_cliente
        )

        self.lineEditBuscador.textChanged.connect(
            self.buscar_cliente
        )

        # Placeholders
        self.InputCedula.setPlaceholderText(
            "Ej: # Cedula"
        )

        self.InputNombre.setPlaceholderText(
            "Ej: Pepito"
        )

        self.InputApellido.setPlaceholderText(
            "Ej: Perex"
        )

        self.InputTelefono.setPlaceholderText(
            "Ej: 3185339876"
        )

        self.InputDireccion.setPlaceholderText(
            "Ej: Calle 1, 123 - Piso 1"
        )

        self.lineEditBuscador.setPlaceholderText(
            "Buscar por C.C, Nombre o Apellido"
        )
    ...
    def navegar_siguiente(self):
        ...
        if actual in self.campos:
            ...
            siguiente = self.campos[
                (index + 1) % len(self.campos)
            ]
    ...
    def actualizar_tabla_clientes(self, rows):
        ...
        try:
            ...
            print(
                f"Error al mostrar tabla clientes: {e}"
            )
    ...
    def cargar_datos_fila(self):
        ...
        for columna in range(
            self.TablaClientes.columnCount()
        ):

            item = self.TablaClientes.item(
                fila_seleccionada,
                columna
            )
    ...
    def validar_campos(self):
        ...
        self.InputTelefono.setValidator(
            validator_telefono
        )
        ...
        validator_cedula = QRegularExpressionValidator(
            rx_cedula,
            self.InputCedula
        )

        self.InputCedula.setValidator(
            validator_cedula
        )

        rx_letras = QRegularExpression(
            r"^[a-zA-Z]+$"
        )
        ...
        self.InputNombre.setValidator(
            validator_letras
        )

        self.InputApellido.setValidator(
    ...
    def eliminar_cliente(self):
        ...
        if not ids:
            ...
            enviar_notificacion(
                "Advertencia",
                "No se seleccionaron clientes para eliminar."
            )
        ...
        if respuesta == QMessageBox.StandardButton.Yes:
            ...
            try:
                ...
                enviar_notificacion(
                    "Éxito",
                    "Cliente(s) eliminado(s) correctamente."
                )
                ...
                enviar_notificacion(
                    "Error",
                    f"Error al eliminar clientes: {e}"
                )
    ...
    def registrar_cliente(self):
        ...
        if (
            ...
            QMessageBox.warning(
                self,
                "Cédula inválida",
                "La cédula debe tener entre 6 y 11 dígitos."
            )
        ...
        try:
            ...
            cliente_existente = obtener_cliente_por_id(
                self.db,
                id_cliente=id_cliente
            )
